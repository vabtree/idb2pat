# Convert IDB to pattern (.pat) file. 
# Corrected for use with IDA 7.6 python 3.x
...
def make_func_sig(config, func):
    ...
    sig += ".." * (32 - (len(sig) // 2))    ## 2 sig += ".." * (32 - (len(sig) / 2))
    ...
    for ref_loc, ref in refs.items():    ## 3 for ref_loc, ref in refs.iteritems():
        name = get_name(ref)
        if name is None or name == "":
            continue
...
def main():
    ...
    if c.pat_append:
        ...
        with open(filename, "wb") as f:
            ...
            f.write("---".encode())
            f.write("\r\n".encode())
    print("Patterns file written..ok")
